...
const REASONS = [
  ...
  },
  {
    icon: TrendingUp,
    color: '#2382ff',
    bg: 'rgba(35,130,255,0.12)',
    title: 'Доступ к полному функционалу',
  ...
  },
  {
    icon: Users,
    color: '#8774e1',
    bg: 'rgba(135,116,225,0.12)',
    title: 'Чистое сообщество',
...
export default function SyncInfoView() {
  const { setCurrentView, syncInfoReturnView } = useAppContext();

  const handleBack = () => {
    hapticImpact('light');
    if (syncInfoReturnView === 'registration') {
      setCurrentView('registration');
    } else {
      // Из попапа — просто идём на маркет
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(160deg, #0d0d12 0%, #0f0f16 60%, #0c0c10 100%)',
    ...
    >
      ...
      {/* Back button — только если пришли из AuthView */}
      {fromAuth && (
        <motion.button
      ...
      <div className="relative z-10 flex flex-col items-center px-5 pt-16 pb-10">
        ...
        >
          {[
            { val: '196K+', label: 'Пользователей' },
            { val: '100%', label: 'Верифицировано' },
            { val: '0', label: 'Скамов' },
          ].map((s, i) => (
